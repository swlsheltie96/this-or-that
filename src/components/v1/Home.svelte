<script>
  ...
  // Hover image cycling
  let hoverImageIndex = {};  // map of list name -> current image index
  let hoverIntervals = {};   // map of list name -> interval id

  function startImageCycle(listName, images) {
    if (!images || images.length <= 1) return;
    hoverImageIndex[listName] = 0;
    hoverImageIndex = hoverImageIndex;
    hoverIntervals[listName] = setInterval(() => {
      hoverImageIndex[listName] = (hoverImageIndex[listName] + 1) % images.length;
      hoverImageIndex = hoverImageIndex;
    }, 500);
  }

  function stopImageCycle(listName) {
    if (hoverIntervals[listName]) {
      clearInterval(hoverIntervals[listName]);
      delete hoverIntervals[listName];
    }
    delete hoverImageIndex[listName];
    hoverImageIndex = hoverImageIndex;
  }
  ...
  onDestroy(() => {
    for (const id of Object.values(hoverIntervals)) {
      clearInterval(id);
    }
  });
</script>
  ...
  {:else}
    {#each displayedLists as list}
      <div
        ...
        on:mouseenter={() => startImageCycle(list.name, list.previewImages)}
        on:mouseleave={() => stopImageCycle(list.name)}
      >
        ...
        <div class="col-pic-image">
          {#if hoverImageIndex[list.name] !== undefined && list.previewImages && list.previewImages.length > 0}
            <img src={list.previewImages[hoverImageIndex[list.name]]} alt="" />
...
<style>
  ...
  .col-date { width: 68px; flex-shrink: 0; }
  .header-date { width: calc(68px + 35px); }
  .col-last-voted { width: 35px; flex-shrink: 0; padding: var(--spacing-sm); }
  .col-pic-image {
    width: 35px; height: var(--cell-height); flex-shrink: 0;
    border-right: var(--border); margin-right: -1px;
    padding: var(--spacing-sm); display: flex; align-items: center;
    justify-content: center; box-sizing: border-box;
  }
  .size-medium .col-pic-image { width: 70px; }
  .size-large .col-pic-image { width: 100px; }
  .header-pic { height: var(--cell-height); }
  .col-pic-image img { width: 25px; height: 25px; object-fit: cover; }
  .size-medium .col-pic-image img { width: 60px; height: 60px; }
  .size-large .col-pic-image img { width: 90px; height: 90px; }
  .col-title { width: 172px; flex-shrink: 0; padding: var(--spacing-xs); }
  .col-len { width: 11px; flex-shrink: 0; position: relative; }
  .rotated-text {
    position: absolute; top: 50%; left: 50%;
    transform: translate(-50%, -50%) rotate(90deg); white-space: nowrap;
  }
  .col-descrip { flex: 1; }
  .col-preview { flex: 1; min-width: 0; padding: 0 var(--spacing-md); }
  .col-look, .col-act { flex-shrink: 0; padding: 3px var(--spacing-xs); }

  .table-row {
    display: flex; width: 100%; border-bottom: var(--border); cursor: pointer;
  }
  ...
  .table-row.size-small { height: 35px; }
  .table-row.size-medium { height: 70px; }
  .table-row.size-large { height: 100px; }
  .table-row > div {
    display: flex; align-items: center; justify-content: center;
    height: 100%; border-right: var(--border); font-family: var(--font-family); box-sizing: border-box;
  }
  .table-row > div:last-child { border-right: none; }
  .table-row .col-date, .table-row .col-last-voted { padding: 0 var(--spacing-sm); }
  .table-row .col-title, .table-row .col-len, .table-row .col-descrip { padding: var(--spacing-xs); }
  .table-row .col-title, .table-row .col-descrip {
    justify-content: flex-start; overflow: hidden; white-space: nowrap;
  }
  .table-row .col-preview {
    padding: var(--spacing-xs); overflow: hidden; white-space: nowrap; justify-content: flex-start;
  }
  .table-row .col-look, .table-row .col-act { padding: 3px; }

  .content-text { font-size: var(--font-size-content); color: var(--color-text-primary); }
  .faded { font-size: var(--font-size-content); color: var(--color-text-faded); }
  .table-row .col-date, .table-row .col-last-voted, .table-row .col-len { font-size: var(--font-size-header); }

  .sort-option { opacity: 0.6; }
  .sort-option:hover { opacity: 1; }
  .action-button.active { background-color: var(--color-black); color: var(--color-white); }
  .loading-cell { width: 100%; padding: var(--spacing-md); justify-content: center; }

  @media (max-width: 740px) {
    .controls-wrapper { overflow-x: auto; }
    .home-container { overflow-x: auto; }
    .content-text, .faded, .action-button { font-size: var(--font-size-content-mobile); }
    .table-row, .table-header { width: fit-content; }
    .table-row .col-preview, .table-header .col-preview { border-right: none; }
    .table-header .col-look, .table-header .col-act,
    .table-row .col-look, .table-row .col-act {
      position: sticky; background-color: var(--color-white); z-index: 1;
    }
    .table-header .col-act, .table-row .col-act { right: 0; }
    .table-header .col-look, .table-row .col-look {
      right: var(--act-col-width, 66px); border-left: var(--border);
    }
    .col-title, .col-descrip, .col-preview { width: 200px; min-width: 100px; }
  }
</style>
